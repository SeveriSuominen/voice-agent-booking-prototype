...
An Azure Application Gateway terminates TLS, enforcing TLS1.2+/1.3 policies and retrieving certificates from Key Vault. The integrated Web Application Firewall inspects HTTP(S) traffic against OWASP Top 10 rules, blocking SQL injection, XSS, and other common web attacks.
...
The serverless Azure Functions model auto-scales to meet demand. With each instance handling 20–40 concurrent sessions, scaling to 100 calls requires only a handful of instances. Real-time STT/TTS streaming and Redis caching keep processing times low, ensuring each user-agent exchange remains under 1.6 seconds.
